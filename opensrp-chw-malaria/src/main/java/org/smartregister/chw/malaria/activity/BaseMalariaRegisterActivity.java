...
// Removed explicit Form dependency to avoid type conflicts across versions
...
public class BaseMalariaRegisterActivity extends BaseRegisterActivity implements MalariaRegisterContract.View {
    ...
    // Implement abstract method from BaseRegisterActivity using Map signature
    @Override
    public void startFormActivity(String formName, String entityId, java.util.Map<String, String> metaData) {
        try {
            if (mBaseFragment instanceof BaseMalariaRegisterFragment) {
                String locationId = Context.getInstance().allSharedPreferences().getPreference(AllConstants.CURRENT_LOCATION_ID);
                // Meta is not used downstream for this module; pass null for metadata string
                presenter().startForm(formName, entityId, null, locationId);
            }
        } catch (Exception e) {
            Timber.e(e);
            displayToast(getString(R.string.error_unable_to_start_form));
